...
import { initialListedProducts } from './productPublish'
...
export const initialFavorites=portalProducts.filter(product=>favoriteNames.includes(product.name)).map((product,index)=>({...product,favoriteAt:`2026-07-${String(15-index).padStart(2,'0')} 10:20`,status:'已上架'}))

const myApplicationSeeds=[
...
export const initialMyApplications=myApplicationSeeds.map((application)=>({...application,applicant:currentUser.name,department:currentUser.department}))
...
export const initialApprovals=approvalSeeds.map((approval)=>({...approval,applicant:currentUser.name,department:currentUser.department,role:currentUser.role}))

export const handledAssets=initialAssets.slice(0,12).map((item,index)=>({...item,duty:index%3===0?'资产负责人':index%3===1?'参与治理':'目录管理员'}))
export const handledProducts=initialListedProducts.map((item,index)=>({...item,duty:index%2?'上架管理':'产品负责人'}))
export const initialApprovalHistory=[
 {id:'AH-001',applicationId:'AP-202607-090',productName:'广东省输电线路路径 GIS 数据集',applicant:currentUser.name,result:'通过',opinion:'用途明确，按项目范围授权',time:'2026-07-12 15:20',period:'半年'},
 {id:'AH-002',applicationId:'AP-202607-091',productName:'粤港澳大湾区数字高程模型',applicant:currentUser.name,result:'通过',opinion:'符合勘测项目使用要求',time:'2026-07-11 10:10',period:'三个月'},
 {id:'AH-003',applicationId:'AP-202607-092',productName:'输电线路杆塔三维模型库',applicant:currentUser.name,result:'驳回',opinion:'需补充模型使用范围',time:'2026-07-10 16:40',period:'—'},
]
