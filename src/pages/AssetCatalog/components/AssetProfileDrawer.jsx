...
function ValuePanel({ asset }) {
  const healthItems=[['数据质量',asset.qualityScore],['更新及时性',asset.health],['元数据完整度',asset.metadataComplete?96:68],['血缘完整度',92],['安全合规度',95],['服务可用性',94]]
...
function AssetProfileDrawer({ open, asset, onClose, onUpdate, onMove, onPublish, onCancelPublish, onRollback }) {
  const { message } = App.useApp()
