import { CloseCircleOutlined, EditOutlined, PauseCircleOutlined, PlayCircleOutlined, RollbackOutlined, UploadOutlined } from '@ant-design/icons'
...
function reason(asset, action) {
  ...
  const labels = { publish: '当前状态不可申请上架', cancelPublish: '当前资产没有待取消的上架申请', enable: '仅待启用资产可以启用', disable: '仅待申请上架资产可以停用', rollback: '仅治理异常资产可以退回' }
...
function AssetTable({ data, onView, onPublish, onToggle, onCancelPublish, onRollback }) {
  const columns = [
    ...
    { title:'操作', key:'action', fixed:'right', width:206, render:(_,asset)=><div className="table-icon-actions">
      ...
      <TableIconButton label="启用" disabledReason={reason(asset,'enable')} icon={<PlayCircleOutlined />} onClick={()=>onToggle(asset)} />
      <TableIconButton label="停用" disabledReason={reason(asset,'disable')} danger icon={<PauseCircleOutlined />} onClick={()=>onToggle(asset)} />
