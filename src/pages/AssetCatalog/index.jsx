...
import { ASSET_STATUS, canAssetAction, getCatalogPath, initialAssets } from '../../mock/assetCatalog'
import { getAssetStatusOverrides, getReturnedAssetIds, getSubmittedAssets, markAssetReturned, setAssetStatusOverride } from '../../utils/catalogSession'
...
function AssetCatalog() {
  ...
  const [assets, setAssets] = useState(() => {
    ...
    const returnedIds = getReturnedAssetIds()
    const merged = new Map(initialAssets.filter((asset) => !returnedIds.has(asset.id)).map((asset) => [asset.id, enrich({ ...asset, status: statusOverrides.get(asset.id) || asset.status })]))
  ...
  const updateAsset = (next) => {
    setAssets((items) => items.map((item) => item.id === next.id ? enrich(next) : item))
  ...
  const rollback = (asset) => modal.confirm({
    ...
    onOk() {
      markAssetReturned(asset.id, asset.governanceTaskId)
      setAssets((items) => items.filter((item) => item.id !== asset.id))
      setSelectedAssetId(null)
      message.success('已将治理异常数据退回重新治理')
    },
  })
