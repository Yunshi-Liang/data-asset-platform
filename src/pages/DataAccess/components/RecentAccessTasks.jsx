...
function RecentAccessTasks({ tasks, retryingIds, onDetail, onRetry }) {
  const columns = [
    ...
    { title: '操作', key: 'action', width: 150, fixed: 'right', render: (_, record) => <Space size={2}><Button type="link" size="small" icon={<EyeOutlined />} onClick={() => onDetail(record)}>日志</Button>{['failed', 'partial'].includes(record.status) && <Button type="link" size="small" icon={<RedoOutlined />} loading={retryingIds.has(record.id)} onClick={() => onRetry(record)}>重试</Button>}</Space> },
  ]
