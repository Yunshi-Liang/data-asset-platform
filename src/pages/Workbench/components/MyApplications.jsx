...
function MyApplications({ data, highlightId, onView, onWithdraw, onEdit }) {
  const actionColumns = (record) => {
    ...
    const editable = ['已驳回', '已过期', '草稿'].includes(record.status)
  ...
  columns.push({ title:'申请状态', dataIndex:'status', width:100, render:(value)=><Tag color={colors[value]}>{value}</Tag> }, { title:'操作', fixed:'right', width:118, render:(_,record)=>actionColumns(record) })
