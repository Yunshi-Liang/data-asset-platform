...
function Workbench(){
 ...
 const [favorites,setFavorites]=useState(initialFavorites),[applications,setApplications]=useState(initialMyApplications),[approvals,setApprovals]=useState(initialApprovals),[history,setHistory]=useState(initialApprovalHistory),[completed,setCompleted]=useState(11)
 ...
 const tabItems=[
  {key:'favorites',label:`我的收藏 ${favorites.length}`,children:<FavoriteProducts data={favorites} onView={setPortalProduct} onApply={setApplicationProduct} onRemove={record=>{setFavorites(items=>items.filter(item=>item.id!==record.id));message.success('已取消收藏')}}/>},
  {key:'applications',label:`我的申请 ${applications.length}`,children:<MyApplications data={applications} onView={setApplicationDetail} onWithdraw={withdraw} onReapply={setApplicationProduct} onAccess={record=>message.success(`${record.method}权限可用，当前为 Demo 模拟入口`)}/>},
