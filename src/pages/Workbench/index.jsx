...
function Workbench(){
 ...
 const tabItems=[
  {key:'favorites',label:`我的收藏 ${favorites.length}`,children:<FavoriteProducts data={favorites} onView={setPortalProduct} onRemove={record=>{setFavorites(items=>items.filter(item=>item.id!==record.id));message.success('已取消收藏')}}/>},
  {key:'applications',label:`我的申请 ${applications.length}`,children:<MyApplications data={applications} highlightId={highlightId} onView={setApplicationDetail} onWithdraw={withdraw} onReapply={setApplicationProduct}/>},
 ...
 return <div className="workbench-page">{modalContextHolder}<div className="workbench-title"><Typography.Title level={3}>个人工作台</Typography.Title><Typography.Text type="secondary">集中查看数据收藏、使用申请、待办审批及个人经手的数据资产。</Typography.Text></div><WorkbenchOverview favorites={favorites.length} applications={processingCount} pendingApprovals={pendingCount} completedApprovals={completed} handledProductCount={handledProducts.length}/><Card className="workbench-tabs"><Tabs activeKey={activeTab} onChange={setActiveTab} items={tabItems}/></Card>
  <ProductDetailDrawer product={portalProduct} open={Boolean(portalProduct)} favorite onClose={()=>setPortalProduct(null)} onFavorite={()=>{}} onApply={setApplicationProduct}/>
