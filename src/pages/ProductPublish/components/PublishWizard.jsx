...
function AssetStep({asset,onReselect}){
 ...
 return <><Alert showIcon type="info" message="已选择治理完成的数据资产作为产品封装来源"/><Descriptions className="wizard-block" bordered column={2} items={fields.map(([label,children],i)=>({key:i,label,children,span:label==='资产名称'?2:1}))}/><Button onClick={onReselect}>重新选择资产</Button></>
}
...
function QualityReview({asset}){
 ...
 const passed=checks.every(v=>v[1])
 return <><Alert type={passed?'success':'error'} showIcon message={passed?'自动质量审核通过，可以进入安全审核':'质量审核未通过，当前产品不能发布'}/><Row gutter={16} className="wizard-block"><Col span={7}><Card><Progress type="dashboard" percent={asset.qualityScore}/></Card></Col><Col span={17}><div className="review-list">{checks.map(([name,pass,detail])=><div className={pass?'review-pass':'review-fail'} key={name}><span>{pass?<CheckCircleFilled/>:'✕'} {name}</span><strong>{detail}</strong></div>)}</div></Col></Row><Button onClick={()=>setDetailOpen(true)}>查看审核详情</Button><Modal open={detailOpen} title="质量审核详情" footer={<Button type="primary" onClick={()=>setDetailOpen(false)}>关闭</Button>} onCancel={()=>setDetailOpen(false)}>资产质量评分 {asset.qualityScore} 分；元数据、血缘、标签和未解决问题均已完成自动检查。</Modal></>
}
function SecurityReview({asset}){
 const forbidden=asset.securityLevel==='核心数据', conclusion=forbidden?'禁止公开':asset.securityLevel==='重要数据'?'允许授权发布':'允许发布'
 return <><Alert type={forbidden?'error':asset.securityLevel==='重要数据'?'warning':'success'} showIcon message={`安全审核结论：${conclusion}`} description={forbidden?'核心数据不可直接进入公开数据资产门户。':'已根据安全等级自动匹配开放与审批策略。'}/><Row gutter={12} className="wizard-block">{[['安全等级',asset.securityLevel],['脱敏状态','已完成'],['权限校验','通过'],['敏感字段',asset.securityLevel==='重要数据'?'3 个':'0 个'],['开放方式',asset.securityLevel==='一般数据'?'按配置开放':'授权访问'],['审核结论',conclusion]].map(([title,value])=><Col span={8} key={title}><Card><Statistic title={title} value={value}/></Card></Col>)}</Row></>
}
function PermissionForm({form,asset}){
 return <Form form={form} layout="vertical" initialValues={{visibility:asset.securityLevel==='一般数据'?'全部用户':'指定角色',departments:'输电工程设计中心、勘测工程中心',roles:'专业设计人员、项目经理',projects:'已授权华南区域电力工程',methods:asset.dataType==='API 服务'?['API 调用','数据推送']:['文件下载','在线查看'],approval:asset.securityLevel==='一般数据'?'无需审批':'管理员审批',validity:'一年'}}>
  <Form.Item name="visibility" label="产品可见范围" rules={formRules}><Radio.Group options={visibilityOptions}/></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="departments" label="指定部门"><Input/></Form.Item></Col><Col span={12}><Form.Item name="roles" label="指定角色"><Input/></Form.Item></Col></Row><Form.Item name="projects" label="指定项目"><Input/></Form.Item>
  <Form.Item name="methods" label="数据使用方式" rules={formRules}><Checkbox.Group options={accessOptions}/></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="approval" label="审批方式" rules={formRules}><Select options={options(approvalOptions)}/></Form.Item></Col><Col span={12}><Form.Item name="validity" label="有效期" rules={formRules}><Select options={options(validityOptions)}/></Form.Item></Col></Row>
 </Form>
}
function PublishPreview({asset,product,permission,onPreview}){
 return <><div className="portal-preview"><div><Tag color="blue">{product.domain}</Tag><Tag color="orange">{asset.securityLevel}</Tag><Typography.Title level={3}>{product.name}</Typography.Title><Typography.Paragraph>{product.description}</Typography.Paragraph><Space wrap>{product.tags?.split('、').map(tag=><Tag key={tag}>{tag}</Tag>)}</Space></div><Divider/><Descriptions column={2} items={[['资产来源',asset.name],['所属地区',product.region],['数据格式',product.format],['质量评分',`${asset.qualityScore} 分`],['更新时间',asset.updatedAt],['申请方式',permission.approval],['可见范围',permission.visibility],['使用方式',permission.methods?.join('、')]].map(([label,children],i)=>({key:i,label,children}))}/></div>{onPreview&&<Button icon={<EyeOutlined/>} onClick={onPreview}>产品门户预览</Button>}</>
}

function PublishWizard({open,asset,onClose,onPublished}){
 const [current,setCurrent]=useState(0),[productForm]=Form.useForm(),[permissionForm]=Form.useForm(),[product,setProduct]=useState({}),[permission,setPermission]=useState({}),[result,setResult]=useState(null),[preview,setPreview]=useState(false)
 useEffect(()=>{if(open){setCurrent(0);setResult(null);setProduct({});setPermission({})}},[open,asset?.id])
 if(!asset)return null
 const qualityPassed=asset.qualityScore>=80&&asset.metadataComplete&&asset.unresolvedIssues<5, securityPassed=asset.securityLevel!=='核心数据'
 const next=async()=>{if(current===1)setProduct(await productForm.validateFields());if(current===4)setPermission(await permissionForm.validateFields());setCurrent(v=>v+1)}
 const publish=()=>{const code=`DP-2026-${String(Date.now()).slice(-4)}`,publishedAt=new Date().toLocaleString('zh-CN',{hour12:false});const newProduct={id:code,code,name:product.name,assetName:asset.name,publishedAt,views:0,applications:0,favorites:0,downloads:0,apiCalls:0,status:'已发布',owner:product.owner,domain:product.domain,region:product.region,format:product.format,qualityScore:asset.qualityScore,permission:permission.approval,accessMethods:permission.methods,description:product.description};setResult(newProduct);onPublished(newProduct)}
 const items=['选择资产','产品信息配置','质量审核','安全审核','权限配置','发布确认'].map(title=>({title}))
 ...
 return <><Drawer open={open} onClose={onClose} width="min(1080px,95vw)" title="数据产品发布工作区" extra={!result&&<Tag icon={<SafetyCertificateOutlined/>} color="blue">Mock 审核流程</Tag>}>
  {result?<Result status="success" title="数据产品发布成功" subTitle={`产品编号：${result.code}`} extra={[<Button type="primary" key="portal" onClick={()=>onPublished(result,true)}>查看门户</Button>,<Button key="list" onClick={onClose}>返回列表</Button>,<Button key="continue" onClick={()=>{setResult(null);setCurrent(0)}}>继续发布</Button>]}><Descriptions bordered column={1} items={[{key:1,label:'发布时间',children:result.publishedAt},{key:2,label:'发布人',children:'数据管理员'},{key:3,label:'门户地址',children:'数据资产门户'}]}/></Result>:<><Steps current={current} items={items}/><div className="wizard-content">{content}</div><div className="wizard-footer"><Button disabled={current===0} onClick={()=>setCurrent(v=>v-1)}>上一步</Button>{current<5?<Button type="primary" disabled={(current===2&&!qualityPassed)||(current===3&&!securityPassed)} onClick={next}>下一步</Button>:<Button type="primary" onClick={publish}>发布产品</Button>}</div></>}
 </Drawer><Modal open={preview} width={720} title="数据资产门户产品预览" footer={<Button type="primary" onClick={()=>setPreview(false)}>关闭预览</Button>} onCancel={()=>setPreview(false)}>{product.name&&<PublishPreview asset={asset} product={product} permission={permission}/>}</Modal></>
}
