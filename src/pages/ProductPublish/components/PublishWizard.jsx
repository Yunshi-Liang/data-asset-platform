...
function QualityReview({ asset }) {
  ...
  return <><Alert type={passed?'success':'error'} showIcon message={passed?'自动质量检查通过，可以进入安全检查':'质量检查未通过，当前产品不能上架'} /><Row gutter={[16,16]} className="wizard-block quality-review-layout"><Col span={7}><Card className="quality-score-card"><Progress type="dashboard" percent={asset.qualityScore} /><Button onClick={() => setDetailOpen(true)}>检查详情</Button></Card></Col><Col span={17}><div className="review-list">{checks.map(([name,pass,detail]) => <div className={pass?'review-pass':'review-fail'} key={name}><span>{pass?<CheckCircleFilled />:'✕'} {name}</span><strong>{detail}</strong></div>)}</div></Col></Row><Modal open={detailOpen} title="质量检查详情" footer={<Button type="primary" onClick={() => setDetailOpen(false)}>关闭</Button>} onCancel={() => setDetailOpen(false)}>资产质量评分 {asset.qualityScore} 分；元数据、血缘、标签和未解决问题均已完成自动检查。</Modal></>
}
...
function PublishPreview({ asset, product, permission, onPreview }) {
  return <Space direction="vertical" size="large" className="wizard-step-stack"><div className="portal-preview"><div><Space size={[8,8]} wrap><Tag color="blue">{product.domain}</Tag><Tag color="orange">{asset.securityLevel}</Tag></Space><Typography.Title level={3}>{product.name}</Typography.Title><Typography.Paragraph>{product.description}</Typography.Paragraph><Space size={[8,8]} wrap>{product.tags?.split('、').map((tag) => <Tag key={tag}>{tag}</Tag>)}</Space></div><Divider /><Descriptions column={2} items={[['资产来源',asset.name],['所属地区',product.region],['数据格式',product.format],['质量评分',`${asset.qualityScore} 分`],['更新时间',asset.updatedAt],['申请方式',permission.approval],['可见范围',permission.visibility],['使用方式',permission.methods?.join('、')]].map(([label,children],index) => ({ key:index,label,children }))} /></div>{onPreview && <Button icon={<EyeOutlined />} onClick={onPreview}>产品门户预览</Button>}</Space>
}
