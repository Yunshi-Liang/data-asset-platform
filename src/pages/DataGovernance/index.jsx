...
import { getGovernanceTaskOverrides, registerSubmittedAsset } from '../../utils/catalogSession'
...
function DataGovernance() {
  ...
  const [tasks, setTasks] = useState(() => {
    const overrides = getGovernanceTaskOverrides()
    return governanceTasks.map((task) => ({ ...task, ...(overrides.get(task.id) || {}) }))
  })
  ...
  const saveAndExit = (id) => {
    const status = drafts[id].currentStep >= 4 ? 'confirming' : 'running'
  ...
  return (
    <div className="data-governance-page">
      ...
      <Card className="governance-task-card" title={<Space orientation="vertical" size={0}><span>治理任务</span><Text type="secondary">覆盖待治理、执行中和历史治理任务</Text></Space>} extra={<Text type="secondary">当前显示 {filteredTasks.length} / {tasks.length}</Text>}>
        ...
        <GovernanceTaskTable tasks={filteredTasks} onOpen={(task) => setSelectedTaskId(task.id)} onSubmitCatalog={submitCatalog} />
      </Card>
      <GovernanceWorkspaceDrawer open={Boolean(selectedTask)} task={selectedTask} draft={selectedTask ? drafts[selectedTask.id] : null} onDraftChange={(patch) => updateDraft(selectedTask.id, patch)} onClose={() => setSelectedTaskId(null)} onSaveExit={() => saveAndExit(selectedTask.id)} onComplete={() => completeTask(selectedTask.id)} onSubmitCatalog={() => submitCatalog(selectedTask)} onGoCatalog={() => navigate(`/asset-catalog${selectedTask?.assetCode ? `?assetId=${selectedTask.assetCode}` : ''}`)} />
    </div>
  )
}
