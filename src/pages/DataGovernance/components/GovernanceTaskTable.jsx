...
function GovernanceTaskTable({ tasks, onOpen, onSubmitCatalog }) {
  const columns = [
    ...
    { title: '操作', key: 'action', width: 170, fixed: 'right', render: (_, record) => <Space size={0} wrap><Button type="link" onClick={() => onOpen(record)}>{record.status === 'pending' ? '开始治理' : record.status === 'completed' ? '结果' : '继续处理'}</Button>{record.status === 'completed' && record.catalogSubmission === 'unsubmitted' && <Button type="link" onClick={() => onSubmitCatalog(record)}>提交</Button>}{record.catalogSubmission === 'failed' && <Button type="link" onClick={() => onSubmitCatalog(record)}>重试提交</Button>}</Space> },
  ]
