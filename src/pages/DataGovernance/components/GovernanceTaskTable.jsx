...
function GovernanceTaskTable({ tasks, onOpen }) {
  const columns = [
    ...
    { title: '操作', key: 'action', width: 100, fixed: 'right', render: (_, record) => <Button type="link" onClick={() => onOpen(record)}>{record.status === 'pending' || record.status === 'failed' ? '开始治理' : record.status === 'completed' ? '查看结果' : '继续处理'}</Button> },
  ]
  return <Table rowKey="id" columns={columns} dataSource={tasks} scroll={{ x: 1570 }} pagination={{ pageSize: 6, showSizeChanger: false, showTotal: (total) => `共 ${total} 条任务` }} />
}
